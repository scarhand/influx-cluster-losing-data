...
curl -G "http://$IP2:8086/query?pretty=true" --data-urlencode "db=mydb" --data-urlencode "q=SELECT * FROM cpu_load_short"
echo -e ""
echo -e "${RED}${BOLD}This should yield a 'database not found' error${NC}"
...
curl -G "http://$IP2:8086/query?pretty=true" --data-urlencode "db=mydb" --data-urlencode "q=SELECT * FROM cpu_load_short"
echo -e ""
echo -e "${RED}${BOLD}This should yield an empty result, even though we've inserted data in the first run.${NC}"
...
echo -e "$RESULT2"
echo -e ""
echo -e "${RED}${BOLD}This time we will get a result, it'll show the extra data and the old data but without the datapoint with region 'very-old'.${NC}"